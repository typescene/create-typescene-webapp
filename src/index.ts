...
function main() {
  try {
    ...
    if (fs.existsSync(config.name)) {
      ...
      console.log(chalk.red.bold("Using existing folder") + " " + chalk.blue(config.name));
    } else {
    ...
    npm.on("close", code => {
      ...
      if (config.git) {
        ...
        tips.add(
          chalk.bold("Did not") + " initialize a repository",
          "    " + chalk.green("cd " + config.folder),
